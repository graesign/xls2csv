...
# print "This is using python-version: " , (sys.version)
...
def xls2csv(infilepath, outfile, sheetid=1, delimiter=",", sheetdelimiter="--------", encoding="utf-8"):
    # Use QUOTE_NONE for no quoting of cell
    writer = csv.writer(outfile, dialect='excel', quoting=csv.QUOTE_NONE, delimiter=delimiter)
...
def sheet_to_csv(book, sheetid, writer, formats):
    ...
    # Debug
    keys = [sheet.cell(0,4).value for col_index in xrange(sheet.ncols)]

    # Get value name of row 0, column 5
    # (starting counting from 0)
    angele = (sheet.cell(0,4).value)

    # print angele, "\nColumn name is: ", keys, "\n\n\n"

    # Cange the VALUE of a CELL_TEXT
    # if sheet.cell(0, 4).value:
    #     sheet._cell_types[0][4] = xlrd.XL_CELL_TEXT
    #     sheet._cell_values[0][4] = "Charge_Point_Address"
    ...
    for i in xrange(sheet.nrows):
        ...
        # columnn = sheet.row(0), sheet.cell(4)
